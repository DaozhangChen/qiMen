...
function doorAnalyseFunc(curInfo, curArea, doorInfo = baseDoorAnalyseInfo, doorAndStemInfo = baseDoorAndStemAnalyseInfo) {
    const result = []
    ...
    if (targetInfo.door && targetInfo.baseDoor) {
        //门干克应
        const targetAnalyse = doorAndStemInfo.find(item => item.door === targetInfo.door && item.stem === targetInfo.upStem)
        if (targetAnalyse) {
...
export default doorAnalyseFunc