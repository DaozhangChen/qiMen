...
function Sudoku() {
    ...
    // }
    const setCurArea = (event) => {
        const curKey = event.target.getAttribute('data-key')
        curArea.current = curKey
    }
    useEffect(() => { }, [curArea.current])
            ...
            <div className="wrapper">
                <div>
                    ...
                    <div className="sudoku-grid">
                        {/* 生成九宫格 */}
                        {regionArr.map((value) => (
                            <div key={value.number} className="sudoku-row" data-key={value.number} onClick={setCurArea}>
                                {
                ...
                <AnalyseTab curMainInfo={detailInfo} curArea={curArea.current} />
            </div>
        </>
    )
}
